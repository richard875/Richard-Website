...
import {
  ...
  GLOBAL_FRAGMENT_SHADER,
  DEPTH_KEEPALIVE_FRAGMENT_SHADER,
} from "./shader";
import cloudTexture from "../../../static/models/cloud.png";

// Keeps postprocessing's shared depth texture allocated for the entire
// lifetime of the EffectComposer below - DepthOfField is the only effect in
// this scene that ever needs depth, and it mounts/unmounts with day/night;
...
const Model = React.memo(() => {
  ...
  return (
    <React.Suspense fallback={null}>
      ...
      <EffectComposer>
        {[
          // See DepthKeepAlive above - keeps the shared depth texture
          // allocated permanently so DepthOfField mounting/unmounting below
          // never re-triggers the allocation that was corrupting the render.
          ...
          // it into the same shared EffectPass as Bloom/etc and has no
          // cheaper "disabled but present" state. Clamping its props at
          // night instead of unmounting it paid that full per-frame cost for
          // nothing, so it's skipped outright at night rather than dialed
          // down - safe to do now that the Depth placeholder above keeps the
          // depth texture stable across the mount/unmount.
          !effectiveIsNight && (
            <DepthOfField
              key="dof"
